...
bool processScalers(const o2::mid::ChannelScalers& scalers, unsigned long nEvents, double threshold)
{
  auto masks = o2::mid::makeMasks(scalers, nEvents, threshold);
...
int main(int argc, char* argv[])
{
  ...
  isOk &= processScalers(scalers[0], nEvents, threshold);

  o2::mid::FetToDead fetToDead;
  fetToDead.process(aggregator.getData(o2::mid::EventType::FET));

  for (auto& dead : fetToDead.getData()) {
    scalers[1].count(dead);
  }
  std::cout << "\nCHECKING DEAD CHANNELS:" << std::endl;
  isOk &= processScalers(scalers[1], nEvents, threshold);
