...
class ChannelScalers
{
 ...
 private:
  ...
  inline uint32_t getChannelId(uint8_t deId, uint8_t columnId, int lineId, int strip, int cathode)
  {
    /// Returns a unique ID for the channel
    return strip | (lineId << 4) | (cathode << 6) | (static_cast<uint32_t>(columnId) << 8) | (static_cast<uint32_t>(deId) << 12);
  }
