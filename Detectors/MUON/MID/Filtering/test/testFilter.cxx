...
BOOST_AUTO_TEST_CASE(maskMaker)
{
  ...
  for (auto col : data) {
    cs.reset();
    cs.count(col);
    auto masks = o2::mid::makeMasks(cs, 1, 0.);
    auto maskVec = masks.getMasks();
    BOOST_TEST(maskVec.size() == 1);
    for (auto mask : maskVec) {
      for (int iline = 0; iline < 4; ++iline) {
...
BOOST_AUTO_TEST_CASE(FETConversion)
{
  ...
  std::vector<ColumnData> fets;
  fets.push_back({3, 0});
  fets.back().setBendPattern(fullPattern & ~0x1, 0);
  fets.back().setBendPattern(fullPattern, 1);
  fets.back().setBendPattern(fullPattern, 2);
  // The following does not exist in deId 3, col 0
  fets.back().setBendPattern(fullPattern & ~0x1, 3);
  fets.back().setNonBendPattern(fullPattern);

  FetToDead fetToDead;
  fetToDead.process(fets);

  auto inverted = fetToDead.getData();
  BOOST_TEST(inverted.size() == 1);
  BOOST_TEST(inverted.back().deId == fets.back().deId);
  BOOST_TEST(inverted.back().columnId == fets.back().columnId);
