...
class UserLogicCheckerDeviceDPL
{
 public:
  ...
  void run(o2::framework::ProcessingContext& pc)
  {
    ...
    std::vector<LocalBoardRO> bareDecoded, ulDecoded;
    std::vector<ROFRecord> bareRofs, ulRofs;
    for (auto& subSpec : mBareSubSpecs) {
      auto offset = bareDecoded.size();
      const auto tmpDecoded = pc.inputs().get<gsl::span<LocalBoardRO>>(fmt::format("mid_decoded_bare_{}", subSpec));
    ...
    for (auto& subSpec : mULSubSpecs) {
      ...
      const auto tmpDecoded = pc.inputs().get<gsl::span<LocalBoardRO>>(fmt::format("mid_decoded_ul_{}", subSpec));
