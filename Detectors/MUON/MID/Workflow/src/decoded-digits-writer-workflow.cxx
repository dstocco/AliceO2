// Copyright CERN and copyright holders of ALICE O2. This software is
// distributed under the terms of the GNU General Public License v3 (GPL
// Version 3), copied verbatim in the file "COPYING".
//
// See http://alice-o2.web.cern.ch/license for full licensing information.
//
// In applying this license CERN does not waive the privileges and immunities
// granted to it by virtue of its status as an Intergovernmental Organization
// or submit itself to any jurisdiction.

/// \file   MID/Workflos/src/raw-to-digits.cxx
/// \brief  MID raw to digits workflow
/// \author Diego Stocco <Diego.Stocco at cern.ch>
/// \date   22 September 2020

#include <string>
#include <vector>
#include "Framework/Variant.h"
#include "CommonUtils/ConfigurableParam.h"
#include "DPLUtils/MakeRootTreeWriterSpec.h"
#include "DataFormatsMID/ROFRecord.h"
#include "MIDSimulation/ColumnDataMC.h"
#include "MIDWorkflow/DecodedDigitConverterSpec.h"
#include "MIDWorkflow/RawDecoderSpec.h"
#include "MIDWorkflow/RawAggregatorSpec.h"

using namespace o2::framework;

// ------------------------------------------------------------------

// we need to add workflow options before including Framework/runDataProcessing
void customize(std::vector<ConfigParamSpec>& workflowOptions)
{
  std::vector<ConfigParamSpec>
    options{
      {"output-filename", VariantType::String, "mid-digits-decoded.root", {"Decoded digits output file"}}};
  workflowOptions.insert(workflowOptions.end(), options.begin(), options.end());
}

// ------------------------------------------------------------------

#include "Framework/ConfigParamSpec.h"
#include "Framework/runDataProcessing.h"

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  auto outputFilename = cfgc.options().get<std::string>("output-filename");
  WorkflowSpec specs;
  specs.emplace_back(o2::mid::getDecodedDigitConverterSpec());
  specs.emplace_back(MakeRootTreeWriterSpec("MIDDigitWriter",
                                            outputFilename.c_str(),
                                            "o2reco",
                                            -1,
                                            MakeRootTreeWriterSpec::BranchDefinition<std::vector<o2::mid::ColumnDataMC>>{InputSpec{"middigits", o2::header::gDataOriginMID, "DECODEDDIGITS"}, "MIDDigit"},
                                            MakeRootTreeWriterSpec::BranchDefinition<std::vector<o2::mid::ROFRecord>>{InputSpec{"midrofrecords", o2::header::gDataOriginMID, "DECODEDDIGITSROF"}, "MIDROFRecords"})());

  return specs;
}
